...
if operation == "Do Multiplication":
    ...
    st.markdown("""
                ...
                across science, engineering, and tech. It provides a compact, computationally efficient way to manage vast datasets and sequential operations""")

    # Puts 2 new empty lines
    st.write("\n\n")

    st.write("Enter matrices below using comma-separated values for elements and newlines for rows.")
    st.write("Example: `1, 2\n3, 4`")
...
# This section manages the algebriac expressions solving
if operation == "Solve algebraic expressions":
    st.title("Solve Algebriac equations using matrix inverse method")
    ...
    st.write("Enter matrices below using comma-separated values for elements and newlines for rows.")
    st.write("Example: Matrix A can be in the form `3, 1\n1, 2` and Matrix B can be in the form `9, 8`. Matrix X will always be represented by `x, y`")
            ...
            with col_inner_1:
                try:
                    ...
                    if det_A == 0:
                        ...
                        X = A_inv @ matrix_b
                        st.subheader("Solution for X (x, y)")
                        
                        # First, flatten the numpy array to 1D and then convert to list
                        X_list = list(X.flatten())

                        st.write("Value is x is:")
                        st.write(round(X_list[0]))

                        st.write("Value is y is:")
                        st.write(round(X_list[1]))

                        # Verification - Check if A @ X equals B
                        B_check = matrix_a @ X
                        st.subheader("\nVerification:  (A * X) should be equal to B:")
                        st.dataframe(pd.DataFrame(B_check))

                        # Show confirmation message
                        st.success(f"The given algebriac equations were successfully solved.")
